...
my %sims = ();
my @values = ();
open COSINE, "../preProcessOutput/".$name."-modSimMetrics.txt";
...
for my $com (keys %communities){
	...
	foreach my $id (@clusterSentIds){
		$clusterSims{$id} = ();
	}

	for my $id1(@clusterSentIds) {
		for my $id2(@clusterSentIds){
				$clusterSims{$id1}{$id2} = $sims{$id1}{$id2};
		}
	}


	$comcluster->{cosine_matrix} = \% clusterSims;
